...
//	fwrite(&chunk, 1, sizeof(XCHUNKHEADER), fpwave);
    ...
    /////////////////
//    unsigned char wav_head[sizeof(RIFFHEADER) + sizeof(XCHUNKHEADER) + sizeof(WAVEFORMATX) + sizeof(XCHUNKHEADER)];
//    unsigned char * pt_buff=wav_head;
//    
//    // 1. 写RIFF头
//	RIFFHEADER riff;
...
//    memcpy(pt_buff, &riff, sizeof(RIFFHEADER));
//    pt_buff += sizeof(RIFFHEADER);
//	
//	// 2. 写FMT块
//	XCHUNKHEADER chunk;
//	memcpy(chunk.chChunkID, "fmt ", 4);
//	chunk.nChunkSize = sizeof(WAVEFORMATX);
//    memcpy(pt_buff, &chunk, sizeof(XCHUNKHEADER));
//    pt_buff += sizeof(XCHUNKHEADER);
//    
//    WAVEFORMATX wfx;
//	memset(&wfx, 0, sizeof(WAVEFORMATX));
//	wfx.nFormatTag = 1;
...
//	wfx.nBlockAlign = 2;
//	wfx.nBitsPerSample = 16; // 16位
//    memcpy(pt_buff, &wfx, sizeof(WAVEFORMATX));
//    pt_buff += sizeof(WAVEFORMATX);
//	
//	// 3. 写data块头
...
// 返回值: 0-出错; 1-正确
int ReadAMRFrame(FILE* fpamr, unsigned char * frameBuffer,int size_frameBuffer, int stdFrameSize, unsigned char stdFrameHeader)
{
//	int bytes = 0;
	unsigned char frameHeader; // 帧头
	
	memset(frameBuffer, 0, size_frameBuffer);
...
// 将AMR文件解码成NSData，以便被播放
NSData *DecodeAMRToNSData(const char* pchAMRFileName)
{
    int headlen = sizeof(RIFFHEADER) + sizeof(XCHUNKHEADER) + sizeof(WAVEFORMATX) + sizeof(XCHUNKHEADER);
    char wav_head[headlen];
    NSMutableData *wav = [[NSMutableData alloc] initWithBytes:wav_head length:headlen];
	FILE* fpamr = NULL;
	char magic[8];
	void * destate;
	int nFrameCount = 0;
	...
	// 读首帧
	amrFrame[0] = stdFrameHeader;
	fread(&(amrFrame[1]), 1, (stdFrameSize-1)*sizeof(unsigned char), fpamr);
	if (feof(fpamr))
    {
        return 0;
    }
	...
	nFrameCount++;
    [wav appendBytes:pcmFrame length:sizeof(short)*PCM_FRAME_SIZE];

	// 逐帧解码AMR并写到WAVE文件里
	while(1)
	{
		memset(pcmFrame, 0, sizeof(pcmFrame));
		if (!ReadAMRFrame(fpamr, amrFrame,MAX_AMR_FRAME_SIZE, stdFrameSize, stdFrameHeader))
        {
            break;
		}
    ...
    [wav replaceBytesInRange:range withBytes:wav_head];
