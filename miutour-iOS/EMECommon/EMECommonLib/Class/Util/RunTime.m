...
//            NIF_INFO(@"reVal=%@",[reVal class]);
            if (reVal) {
                if ([self isSuperClass:[BaseModelClass class] class:[reVal class]]) {
...
+(NSDictionary *)NSDictionaryFromAttributeWithInstance:(id)instance
{
    ...
    for (NSInteger i=0; i<[list count]; i++) {
        ...
        if ([data isKindOfClass:[BaseModelClass class]]) {
        ...
        for (NSInteger i=0; i<[allKeys count]; i++) {
            ...
            if (type.length>0) {
                ...
                if ([value isKindOfClass:[NSArray class]] && [type rangeOfString:@"@\"NSArray" options:NSLiteralSearch].location==0) {
                    ...
                    id object;
                    if ([@"Account" isEqualToString:type]) {
                        object=[[Account alloc] init];
                    }else{
