...
@interface MTActivityViewController ()<UITableViewDataSource,UITableViewDelegate,EMEBaseDataManagerDelegate,MTIdentityManagerDelegate>
...
@implementation MTActivityViewController
...
- (void)efQueryMessageList
{
    ...
    [[MTOrderHttpRequestDataManager shareInstance] efQueryActivityListWithUsername:[UserManager shareInstance].user.loginName token:[UserManager shareInstance].user.token pageNo:@"1" pageSize:@"9999"];
}
...
- (void)didFinishLoadingJSONValue:(NSDictionary *)dic URLConnection:(EMEURLConnection *)connection
{
    ...
    if (connection.connectionTag == TagForActivitylist) {
