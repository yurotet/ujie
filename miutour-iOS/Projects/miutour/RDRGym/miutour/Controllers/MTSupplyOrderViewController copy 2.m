...
@interface MTSupplyOrderViewController ()<MTRefreshTableViewDataSource,MTRefreshTableViewDelegate,EMEBaseDataManagerDelegate>

@property (nonatomic,strong) MTRefreshTableView *orderTableView;
...
- (UITableView *)orderTableView
{
    if (_orderTableView == nil) {
        ...
        _orderTableView = [[MTRefreshTableView alloc] initWithFrame:etFrame style:UITableViewStylePlain];
...
#pragma mark -
#pragma MTTableView Delegate

-(void)refreshTableViewReload:(MTRefreshTableView * )refreshTableView{
    [NSTimer scheduledTimerWithTimeInterval:.5f target:self selector:@selector(loadNewData) userInfo:nil repeats:NO];
}
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView{
    [self.orderTableView refreshTableViewDidScroll:scrollView];
}

- (void)scrollViewDidEndDragging:(UIScrollView *)scrollView willDecelerate:(BOOL)decelerate{
    [self.orderTableView refreshTableViewDidEndDraging:scrollView];
}
...
- (void)didFinishLoadingJSONValue:(NSDictionary *)dic URLConnection:(EMEURLConnection *)connection
{
    ...
    if (connection.connectionTag == TagForOrderList) {
        ...
        [self.orderTableView refreshTableViewDidFinishReload];
